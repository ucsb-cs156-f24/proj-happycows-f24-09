...
describe("PagedProfitsTable tests", () => {
  ...
  test("Values are formatted and aligned correctly", () => {
    ...
    // Get the cells by testid
    const amountCell = screen.getByTestId(`${testId}-cell-row-0-col-amount`);
    ...
    // Check content
    expect(amountCell).toHaveTextContent("$100.00");
    expect(healthCell).toHaveTextContent("95.00%");
    expect(cowsCell).toHaveTextContent("10");

    // Check alignment
    expect(amountCell.firstChild).toHaveStyle("text-align: right");
    expect(healthCell.firstChild).toHaveStyle("text-align: right");
    expect(cowsCell.firstChild).toHaveStyle("text-align: right");
});
});